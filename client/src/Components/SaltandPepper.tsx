...
function SaltandPepper({ user, updateUser }: Props) {
    ...
    const checkExistingGame = async () => {
        ...
        try {
            const response = await fetch("https://salt-venture.azurewebsites.net/api/saltnpepper", requestSettings)
            if (!response.ok) {
    ...
    const pickItem = async (id) => {
        ...
        try {
            const response = await fetch("https://salt-venture.azurewebsites.net/api/saltnpepper/pick/" + id, requestSettings)
            if (!response.ok) {
                throw new Error(JSON.stringify(await response.json()));
            }
            const deserializedJSON = await response.json();
